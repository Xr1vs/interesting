...
Each person then clicks **"Connect with Roblox"** in the app's Settings tab,
...
**"Roblox connection failed" after clicking Connect** — double-check the
Redirect URI registered on the OAuth app matches `APP_BASE_URL` (or your
actual domain) plus `/api/roblox/oauth-callback` **exactly**, including
`https://` vs `http://`. Roblox requires an exact match.
